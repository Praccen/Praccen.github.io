...
class Heightmap extends Mesh_1.default {
    ...
    calculateVertexNormal(x, z) {
        ...
        let resultingNormal = new Vec3_1.default();
        let middlePos = new Vec3_1.default([
            0.0,
        ...
        for (let i = 0; i < 4; i++) {
            let first = i;
            let second = (i + 1) % 4;
            tempTriangle.setVertices(new Vec3_1.default([
                offsets[first][0] * this.xQuadSize,
                this.vertices[(z + offsets[first][1]) * this.xResolution * 8 +
                    (x + offsets[first][0]) * 8 +
                    1],
                offsets[first][1] * this.zQuadSize,
            ]), middlePos, new Vec3_1.default([
                offsets[second][0] * this.xQuadSize,
                this.vertices[(z + offsets[second][1]) * this.xResolution * 8 +
    ...
    getNormalFromWorldPosition(heightmapTransformMatrix, worldPosition, invertedTransformMatrix) {
        ...
        let transformedPos = invertedTransformMatrix.multiplyVector4(new Vector4([...worldPosition, 1.0]));
    ...
    getHeightFromWorldPosition(heightmapTransformMatrix, worldPosition, invertedTransformMatrix) {
        ...
        // Take the world position and transform it into heightmap local coordinates
        let transformedPos = invertedMatrix.multiplyVector4(new Vector4([...worldPosition, 1.0]));
        // Get the height of the heightmap at the corresponding position
        let height = this.getHeight(transformedPos.elements[0], transformedPos.elements[2]);
        if (height == null) {
            return null;
        }
